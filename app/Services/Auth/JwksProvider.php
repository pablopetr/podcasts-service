...
class JwksProvider
{
    public function getJwks(bool $forceRefresh = false): array
    {
        ...
        return Cache::remember($key, $ttl, function () {
            ...
            if (! $resp->ok()) {
                throw new RuntimeException("Failed JWKS: {$resp->status()}");
            }
    ...
    public function getPublicPemByKid(string $kid): string
    {
        ...
        if (! $pem) {
            $pem = $this->searchPem($this->getJwks(true), $kid);
        } // rotação
        if (! $pem) {
            throw new RuntimeException("No key for kid={$kid}");
        }
